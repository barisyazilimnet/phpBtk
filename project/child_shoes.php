...
$query = $db_con->prepare("SELECT * FROM products WHERE product_type = 'Çocuk Ayakkabısı' AND product_status = 1 $menu_sql $search_sql ORDER BY product_id DESC");
$query->execute();
$total_number_records = $query->rowCount();
$limit = 10;
$start = ($paging * $limit) - $limit;
$total_page_number = ceil($total_number_records / $limit);
$query = $db_con->prepare("SELECT SUM(menu_product_number) AS menus_total_product_count FROM menus WHERE menu_product_type = 'Çocuk Ayakkabısı' ");
$query->execute();
                    ...
                    <td>
                        <table width="250" style="border-collapse:collapse;">
                            ...
                            <tr style="height: 30px;">
                                <td><a style="text-decoration: none; <?php echo ($menu_id == "") ? "color: #f90;" : "color: #646464;"; ?>  font-weight: bold;" href="index.php?page_code=85">&nbsp;Tüm Ürünler ( <?php echo $menus->menus_total_product_count;  ?> )</a></td>
                            </tr>
                            <?php
                            $query = $db_con->prepare("SELECT * FROM menus WHERE menu_product_type = 'Çocuk Ayakkabısı' ORDER BY menu_name");
                            $query->execute();
                            $menus = $query->fetchAll(PDO::FETCH_OBJ);
                            foreach ($menus as $menu) {
                            ?>
                                <tr style="height: 30px;">
                                    <td><a style="text-decoration: none; <?php echo ($menu_id == $menu->menu_id) ? "color: #f90;" : "color: #646464;"; ?>  font-weight: bold;" href="index.php?page_code=85&menu_id=<?php echo $menu->menu_id; ?>">&nbsp;<?php echo decode($menu->menu_name); ?> ( <?php echo $menu->menu_product_number; ?> )</a></td>
                                </tr>
                            <?php
                            }
                            ?>
                ...
                <tr>
                    <td>
                        <div class="search_section">
                            <form action="index.php?page_code=85" method="POST">
                ...
                <tr>
                    <td>
                        <table width="805" style="border-collapse:collapse;">
                            <tr>
                                <?php
                                $query = $db_con->prepare("SELECT * FROM products WHERE product_type = 'Çocuk Ayakkabısı' AND product_status = 1 $menu_sql $search_sql ORDER BY product_id DESC LIMIT $start, $limit");
                                $query->execute();
                                            ...
                                            <tr>
                                                <td>
                                                    <a href="index.php?page_code=82&product_id=<?php echo $product->product_id; ?>">
                                                        <img width="185" height="247" src="img/products/child/<?php echo $product->product_img ?>" alt="<?php echo $product->product_img ?>">
                                                    </a>
                                                </td>
                                            </tr>
                        ...
                        <td style="text-align: center;">
                            <div class="paging_container">
                                ...
                                <div class="paging_container_inside_number_container">
                                    <?php
                                    if ($paging > 1) {
                                        echo '<span class="paging_passive"><a href="index.php?page_code=85&paging=1' . $menu_paging . $search_paging . '"><<</a></span>';
                                        echo '<span class="paging_passive"><a href="index.php?page_code=85&paging=' . ($paging - 1) . $menu_paging . $search_paging . '"><</a></span>';
                                    }
                                    for ($page_index = $paging - 2; $page_index <= $paging + 2; $page_index++) {
                                        if ($page_index > 0 and $page_index <= $total_page_number) {
                                            if ($page_index == $paging) {
                                                ...
                                                echo '<span class="paging_passive"><a href="index.php?page_code=85&paging=' . $page_index . $menu_paging . $search_paging . '">' . $page_index . '</a></span>';
                                            }
                                        }
                                    }
                                    if ($paging != $total_page_number) {
                                        echo '<span class="paging_passive"><a href="index.php?page_code=85&paging=' . ($paging + 1) . $menu_paging . $search_paging . '">></a></span>';
                                        echo '<span class="paging_passive"><a href="index.php?page_code=85&paging=' . $total_page_number . $menu_paging . $search_paging . '">>></a></span>';
                                    }
                                    ?>
                                </div>
                            </div>
