...
if (isset($_POST)) {
    ...
    if ($email != "" and $password != "") {
        $query = $db_con->prepare("SELECT * FROM users WHERE user_email = ? AND user_password = ?");
        ...
        if ($query->rowCount()) {
            if ($user->user_status) {
                ...
                if ($_SESSION["user"] == $email) {
                    header("Location:index.php?page_code=50");
                } else {
                    header("Location:index.php?page_code=33");
                }
            } else {
                $mail_content = "Merhaba Sayın " . $user->user_name_surname . "<br /><br />Sitemizde yapmış olduğunuz üyelik kaydını tamamlamak için lütfen <a href='" . $settings->website_title . "/activation.php?code=" . $user->user_activation_code . "&email=" . $user->user_email . "'>BURAYA TIKLAYINIZ</a><br /><br />Saygılarımızla iyi çalışmalar...<br />" . $settings->website_title;
            ...
            header("Location:index.php?page_code=35");
        }
    } else {
        header("Location:index.php?page_code=34");
    }
}
