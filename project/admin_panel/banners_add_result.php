...
if (isset($_SESSION["admin_user"])) {
    if ($_POST) {
        ...
        $query = $db_con->prepare("INSERT INTO banners SET banner_img=?, banner_name=?, banner_section=?");
        $query->execute([$banner_img_new_name, $banner_name, $banner_section]);
        ...
        if (is_uploaded_file($banner_img["tmp_name"])) {
            ...
            if ($upload_banner_img->uploaded) {
                ...
                if ($upload_banner_img->processed) {
                    ...
                    header("Location:index.php?page_code_outside=0&page_code_inside=37");
                }
            }
        }
        header("Location:index.php?page_code_outside=0&page_code_inside=36");
    } else {
        header("Location:index.php?page_code_outside=0&page_code_inside=37");
    }
} else {
