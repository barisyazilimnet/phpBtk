...
if (isset($_SESSION["admin_user"])) {
    ...
    if ($_POST) {
        ...
        $query = $db_con->prepare("UPDATE shipping_companies SET shipping_company_logo=?, shipping_company_name=? WHERE shipping_company_id=?");
        $query->execute([$shipping_company_logo_new_name, $shipping_company_name, $shipping_company_id]);
        if (is_uploaded_file($shipping_company_logo["tmp_name"])) {
            ...
            if ($upload_shipping_company_logo->uploaded) {
                ...
                if ($upload_shipping_company_logo->processed) {
                    ...
                    header("Location:index.php?page_code_outside=0&page_code_inside=29");
                }
            }
        }
        header("Location:index.php?page_code_outside=0&page_code_inside=28");
    } else {
        header("Location:index.php?page_code_outside=0&page_code_inside=29");
    }
} else {
