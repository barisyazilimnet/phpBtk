...
if (isset($_SESSION["admin_user"])) {
    if ($_POST) {
        ...
        $query = $db_con->prepare("INSERT INTO products SET menu_id=?, product_name=?, product_price=?, product_shipping_price=?, product_currency=?, product_vat_rate=?, product_description=?, product_variant_header=?, product_img=?, product_img_1=?, product_img_2=?, product_img_3=?, product_type=?, product_status=?");
        $query->execute([$menu_id, $name, $price, $shipping_price, $currency, $vat_rate, $description, $variant_header, $img_1_new_name, $img_2_new_name, $img_3_new_name, $img_4_new_name, $mpt->menu_product_type, 1]);
        $added_product_id = $query->last_insert_id();
        $query = $db_con->prepare("INSERT INTO product_variants SET product_id=?, product_variant_name=?, product_variant_stock_quantity=?");
        ...
        $query->execute([$menu_id]);
        header("Location:index.php?page_code_outside=0&page_code_inside=92");
    } else {
        header("Location:index.php?page_code_outside=0&page_code_inside=93");
    }
} else {
