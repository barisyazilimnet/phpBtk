...
if (isset($_SESSION["admin_user"])) {
    ...
    if ($_POST) {
        ...
        $query = $db_con->prepare("UPDATE bank_accounts SET bank_logo=?, bank_name=?, location_city=?, location_country=?, branch_name=?, branch_code=?, currency=?, account_holder=?, account_number=?, iban_number=? WHERE bank_account_id=?");
        $query->execute([$bank_logo_new_name, $bank_name, $bank_branch_name, $bank_branch_code, $bank_location_city, $bank_location_country, $bank_currency, $account_holder, $account_number, $iban_number, $bank_account_id]);
        if (is_uploaded_file($bank_logo["tmp_name"])) {
            ...
            if ($upload_bank_logo->uploaded) {
                ...
                if ($upload_bank_logo->processed) {
                    ...
                    header("Location:index.php?page_code_outside=0&page_code_inside=17");
                }
            }
        }
        header("Location:index.php?page_code_outside=0&page_code_inside=16");
    } else {
        header("Location:index.php?page_code_outside=0&page_code_inside=17");
    }
} else {
